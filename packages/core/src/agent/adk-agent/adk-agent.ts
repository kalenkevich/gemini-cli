...
export class GeminiCliAgent extends BaseAgent {
  ...
  constructor(model: string, private config: Config) {
  ...
  protected async *runAsyncImpl(
    ...
    const llmAgent = buildAdkLlmAgent(this.model, this.config, this.abortController.signal);
...
function buildAdkLlmAgent(model: string, config: Config, abortSignal: AbortSignal) {
  return new LlmAgent({
...
function getAdkTools(config: Config, abortSignal: AbortSignal) {
  ...
  for (const toolId of config.getMainAgentTools() || []) {
    const tool = config.toolRegistry.getTool(toolId);
    if (tool) {
      tools.push(toAdkTool(tool, abortSignal));
    }
  }

  for (const toolId of config.getCoreTools() || []) {
    const tool = config.toolRegistry.getTool(toolId);
    if (tool) {
      tools.push(toAdkTool(tool, abortSignal));
    }
...
function toAdkTool(tool: AnyDeclarativeTool, abortSignal: AbortSignal): FunctionTool {
  return new FunctionTool({
    name: tool.name,
    description: tool.description,
    parameters: tool.schema,
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    execute: async (params) => tool.buildAndExecute(params as object, abortSignal),
  });
}
...
function toAdkMCPToolset(mcpServerConfig: MCPServerConfig, toolPrefix: string) {
  if (mcpServerConfig.type === 'http') {
    ...
    }, mcpServerConfig.includeTools, toolPrefix);
  }
