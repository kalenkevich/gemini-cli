...
import type { AgentEvent, AgentEventType, ContentPart, ElicitationResponse, ToolRequest } from '../types.js';
...
import { geminiPartsToContentParts, buildToolResponseData, toolResultDisplayToContentParts } from '../content-utils.js';
...
export function translateEvent(event: AdkEvent): AgentEvent[] {
  ...
  for (const part of event.content.parts) {
    ...
    if (part.inlineData) {
      result.push(makeEvent('message', {
        role: 'agent',
        content: [{ type: 'media', data: part.inlineData.data, mimeType: part.inlineData.mimeType }],
      }));
    ...
    if (
      ...
      if (response) {
        ...
        const displayContent = toolResultDisplayToContentParts(
          functionResponse.resultDisplay,
        );
        const data = buildToolResponseData(functionResponse);

        result.push(makeEvent('tool_response', {
          requestId: functionId,
          name: part.functionResponse.name,
          content: isError
            ? [{ type: 'text', text: functionResponse.error?.message || '' }]
            : geminiPartsToContentParts(part.functionResponse.parts || []),
          isError,
    ...
    if (
      ...
      result.push(makeEvent('elicitation_request', {
        requestId: functionId,
...
export function elicitationToAdkToolConfirmation(
  ...
  const toolRequestEvent = agentEvents.find((event) => event.type === 'tool_request' && event.requestId === response.requestId);
  if (!toolRequestEvent) {
    throw new Error(`Tool request not found for request id: ${response.requestId}`);
  }
