...
describe('HistorySquashingProcessor', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      getContextManagementConfig: vi.fn().mockReturnValue({
        strategies: {
          historySquashing: { maxTokensPerPrompt: 100 }, // Extremely small limit for testing
        },
      }),
  ...
  const getDummyState = (isSatisfied = false, deficit = 0, protectedIds = new Set<string>()): ContextAccountingState => ({
    currentTokens: 5000,
    maxTokens: 10000,
    retainedTokens: 4000,
  ...
  const createDummyEpisode = (id: string, userText: string, modelThought: string): Episode => ({
    id,
    timestamp: Date.now(),
    trigger: {
      id: randomUUID(),
      ...
      metadata: { originalTokens: 1000, currentTokens: 1000, transformations: [] },
    },
    steps: [
      {
        id: randomUUID(),
  ...
  it('bypasses processing if budget is satisfied', async () => {
    ...
    expect((result[0].trigger as any).semanticParts[0].presentation).toBeUndefined();
  });
  ...
  it('truncates both UserPrompts and AgentThoughts', async () => {
    ...
    const userPart = (result[0].trigger as any).semanticParts[0];
    const thoughtPart = result[0].steps[0] as any;
    
    expect(userPart.presentation).toBeDefined();
    expect(userPart.presentation.text).toContain('[... OMITTED 600 chars ...]');
    
    expect(thoughtPart.presentation).toBeDefined();
    expect(thoughtPart.presentation.text).toContain('[... OMITTED 600 chars ...]');
  ...
  it('stops processing once deficit is resolved', async () => {
    ...
    const ep1Part = (result[0].trigger as any).semanticParts[0];
    expect(ep1Part.presentation).toBeDefined();
    
    // Second episode should be untouched because the deficit hit 0
    const ep2Part = (result[1].trigger as any).semanticParts[0];
