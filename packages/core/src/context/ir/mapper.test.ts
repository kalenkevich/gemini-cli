...
describe('IrMapper', () => {
  it('should correctly map a complex conversation into Episodes and back', () => {
    const rawHistory: Content[] = [
      ...
      { 
        ...
        parts: [
          ...
          { functionCall: { id: 'call_1', name: 'read_file', args: { filepath: 'A.txt' } } },
          { functionCall: { id: 'call_2', name: 'read_file', args: { filepath: 'B.txt' } } }
        ]
      },
      {
        role: 'user',
        parts: [
          { functionResponse: { id: 'call_1', name: 'read_file', response: { output: 'Contents of A' } } },
          { functionResponse: { id: 'call_2', name: 'read_file', response: { output: 'Contents of B' } } }
        ]
      },
      { role: 'model', parts: [{ text: 'Thanks. Now I will compile.' }, { functionCall: { id: 'call_3', name: 'shell', args: { cmd: 'make' } } }] },
      { role: 'user', parts: [{ functionResponse: { id: 'call_3', name: 'shell', response: { output: 'success' } } }] },
    ...
    expect((ep.trigger as any).text).toBe('Can you read file A and B?');
    ...
    expect((ep.steps[1] as any).toolName).toBe('read_file');
    expect((ep.steps[1] as any).intent).toEqual({ filepath: 'A.txt' });
    expect((ep.steps[1] as any).observation).toEqual({ output: 'Contents of A' });
    
    expect(ep.steps[2].type).toBe('TOOL_EXECUTION');
    expect((ep.steps[2] as any).intent).toEqual({ filepath: 'B.txt' });

    expect(ep.steps[3].type).toBe('AGENT_THOUGHT');
    
    expect(ep.steps[4].type).toBe('TOOL_EXECUTION');
    expect((ep.steps[4] as any).toolName).toBe('shell');
    ...
    expect(reconstituted[1]).toEqual({ role: 'model', parts: [{ text: 'Let me check those files.' }] }); // We flushed after thought
